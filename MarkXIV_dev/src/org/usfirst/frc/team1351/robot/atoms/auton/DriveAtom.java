...
public class DriveAtom extends Atom
{
	...
	
	{
		...
		threshold = 503; // we can be within approx. half an inch 
		//CHANGE BACK TO 562 AFTERVMATCH
...
//				Timer.delay(0.5);
		...
		try
		{
			...
			double distanceR = distance + TKOHardware.getRightDrive().getPosition(); 
...
//					TKOHardware.getRightDrive().set(TKOHardware.getRightDrive().getSetpoint() - incrementer);
			...
			TKOHardware.getRightDrive().set(distanceR);
			...
			System.out.println("Position at end of atom: " + TKOHardware.getLeftDrive().getPosition() + "\t" + TKOHardware.getRightDrive().get());
...
//					TKOHardware.getDSolenoid(1).set(Value.kForward);
		...
		System.out.println("D+one executing drive atom");
	}
